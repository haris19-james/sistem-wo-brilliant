...
@section('title', 'Chat')
@section('page-title', 'Chat dengan Tim Brilliant')
@section('page-subtitle', 'Percakapan resmi terikat booking Anda')
...
<div class="max-w-lg mx-auto bg-white rounded-2xl border border-gray-100 p-10 text-center shadow-sm">
    ...
    <a href="{{ route('client.booking.create') }}" class="inline-flex px-6 py-2.5 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition">
        Buat Booking
    </a>
</div>
@else
<div id="customerChatRoot" class="max-w-6xl mx-auto" data-send-url="{{ $selectedPesanan ? route('client.chat.store', $selectedPesanan) : '' }}">
    ...
    <div class="mb-4 flex flex-col sm:flex-row sm:items-center gap-2">
        ...
        <select id="bookingSelect" onchange="if(this.value) window.location='{{ route('client.chat') }}?pesanan_id='+this.value"
            ...
            <option value="{{ $b->id }}" @selected($selectedPesanan?->id == $b->id)>
                {{ $b->nama_pasangan }} — {{ $b->nomor_pesanan }} ({{ $b->tanggal_acara?->format('d M Y') ?? 'TBD' }})
            </option>
            @endforeach
        </select>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 min-h-[calc(100vh-14rem)]">
        ...
        <div class="lg:col-span-8 bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
            ...
            <div class="px-5 py-4 border-b border-gray-100">
                <p class="text-xs text-gray-500 uppercase tracking-wide font-semibold">Chat Resmi · Booking #{{ $thread['booking']['nomor'] }}</p>
                <h2 class="text-lg font-bold text-gray-900 mt-0.5">{{ $thread['booking']['nama_acara'] }}</h2>
            ...
            <div class="flex-1 overflow-y-auto p-5 space-y-4 bg-gray-50/40 min-h-[280px] max-h-[50vh]" id="customerChatMessages">
                ...
                <div class="flex {{ $msg['type'] === 'sent' ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-[88%] sm:max-w-[75%]">
                        <p class="text-[10px] font-semibold mb-1 {{ $msg['type'] === 'sent' ? 'text-right text-green-700' : 'text-gray-500' }}">
                            {{ $msg['sender_label'] }}
                        </p>
                        <div class="px-4 py-2.5 rounded-2xl text-sm shadow-sm {{ $msg['type'] === 'sent' ? 'bg-green-600 text-white rounded-br-md' : 'bg-white border border-gray-100 text-gray-900 rounded-bl-md' }}">
                            <p class="whitespace-pre-wrap leading-relaxed">{{ $msg['text'] }}</p>
                            <div class="flex items-center justify-end gap-1 mt-1.5 {{ $msg['type'] === 'sent' ? 'text-green-100' : 'text-gray-400' }}">
                                <span class="text-[10px]">{{ $msg['time'] }}</span>
                                @if($msg['type'] === 'sent')
                                <span class="inline-flex" title="{{ $msg['read_receipt'] === 'read' ? 'Sudah dibaca tim' : 'Terkirim' }}">
                                    @if($msg['read_receipt'] === 'read')
            ...
            <form id="customerChatForm" method="POST" action="{{ route('client.chat.store', $selectedPesanan) }}" class="p-4 border-t border-gray-100 bg-white" data-no-loading data-ajax>
                @csrf
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sticky top-4">
                ...
                <dl class="space-y-3 text-sm">
                    <div>
                        ...
                        <dd class="font-semibold text-gray-900">{{ $thread['booking']['nama_acara'] }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs text-gray-500">Tanggal Acara</dt>
                        <dd class="font-medium text-gray-800">{{ $thread['booking']['tanggal'] ?? 'Akan ditentukan' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs text-gray-500">Paket</dt>
                        <dd class="text-gray-800">{{ $thread['booking']['paket'] }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs text-gray-500">Status Booking</dt>
                        <dd class="mt-1">
                            <span class="inline-flex px-2.5 py-1 rounded-lg text-xs font-semibold border {{ $thread['booking']['status_class'] }}">
                                {{ $thread['booking']['status_label'] }}
                            </span>
                        </dd>
                    </div>
                    <div>
                        <dt class="text-xs text-gray-500">No. Booking</dt>
                        <dd class="font-mono text-xs text-gray-600">{{ $thread['booking']['nomor'] }}</dd>
                    </div>
                </dl>
                ...
                <a href="{{ route('client.pesanan_detail', $selectedPesanan->id) }}" class="mt-3 inline-block text-xs font-semibold text-green-600 hover:text-green-700">
...
@push('scripts')
<script src="{{ asset('js/customer-chat.js') }}?v=2"></script>
@endpush
