<div class="bg-white border border-amber-100 rounded-3xl p-5 shadow-sm hover:shadow-md transition" data-task-id="{{ $task->id }}" data-task-name="{{ $task->nama_tugas }}">
    <div class="flex items-start justify-between gap-4">
        ...
        <div class="flex flex-col items-end gap-2">
            <span data-status-badge class="inline-flex items-center px-3 py-1 rounded-full bg-amber-50 text-amber-700 text-xs font-semibold">Sedang Dikerjakan</span>
            <select class="task-status-select text-sm px-2 py-1 border border-gray-200 rounded" data-task-id="{{ $task->id }}" data-prev-status="in_progress">
    ...
    <div class="mt-4 grid grid-cols-2 gap-3 text-sm text-gray-600">
        ...
        <div class="space-y-1">
            <p class="font-semibold text-gray-900">Progress</p>
            <p>{{ $task->progress ?? 0 }}%</p>
        </div>
    </div>
