...
@section('title', 'Detail Jadwal Meeting Vendor')
...
<div class="space-y-6">
  ...
  <div class="p-4 rounded-xl border {{ $meeting->status_badge_class }}">
    <div class="flex items-center justify-between">
      <div>
        <p class="font-semibold {{ $meeting->status === 'completed' ? 'text-green-900' : ($meeting->status === 'ongoing' ? 'text-amber-900' : 'text-blue-900') }}">
          Status: {{ $meeting->status_label }}
        </p>
        @if($meeting->isOverdue())
        <p class="text-sm text-red-600 mt-1">⚠️ Meeting ini sudah lewat tanggalnya!</p>
        @elseif($meeting->isToday())
        <p class="text-sm {{ $meeting->status === 'completed' ? 'text-green-600' : 'text-red-600' }} mt-1">
      ...
      <div class="text-right text-sm text-gray-600">
        Dibuat: {{ $meeting->created_at->translatedFormat('d F Y H:i') }}
      </div>
    </div>
  </div>
        ...
        <div class="border-b border-gray-200 pb-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="bg-pink-50 rounded-lg p-4 border border-pink-100">
              ...
              <p class="text-xs text-pink-600 mt-1">{{ $meeting->meeting_date->translatedFormat('l') }}</p>
            </div>
        ...
        <div class="border-b border-gray-200 pb-6">
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-gray-600 mt-1">{{ $meeting->location }}</p>
            </div>
          </div>
        </div>
          ...
          <div class="bg-purple-50 rounded-lg p-4 border border-purple-100">
            <div class="space-y-3">
              ...
              <div>
                ...
                <p class="text-gray-900 mt-1">{{ $meeting->booking->user->name ?? 'N/A' }}</p>
      ...
      <div class="bg-white border border-gray-100 rounded-2xl p-6">
        ...
        <div class="bg-green-50 border border-green-200 rounded-lg p-4">
          ...
          <p class="text-xs text-green-600 mt-3">
            Terakhir update: {{ $meeting->updated_at->translatedFormat('d F Y H:i') }}
          </p>
        </div>
        @elseif($meeting->notes && $meeting->status !== 'completed')
    ...
    <div class="lg:col-span-1">
      <div class="bg-white border border-gray-100 rounded-2xl p-6 sticky top-6 space-y-4">
        ...
        <div class="space-y-3 border-b border-gray-200 pb-4">
          ...
          <form action="{{ route('lapangan.vendor-meetings.updateStatus', $meeting) }}" method="POST" class="space-y-2">
            @csrf
            @method('PATCH')
...
{{-- Modal: Complete Meeting with Notes --}}
@include('lapangan.modules.vendor-meetings.complete_form')
@endsection
