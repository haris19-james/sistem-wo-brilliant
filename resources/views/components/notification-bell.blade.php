...
<div class="relative shrink-0 notification-bell-root" x-data="notificationBell()" x-init="init(@js($initialUnread), @js($initialNotifications))" @click.outside="open = false">
    <button type="button"
        ...
        <span class="notification-bell-badge absolute -top-0.5 -right-0.5 flex min-w-[1.25rem] h-5 items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white ring-2 ring-white"
            ...
            x-text="unreadCount > 99 ? '99+' : unreadCount">{{ $initialUnread > 99 ? '99+' : $initialUnread }}</span>
        @else
    ...
    <div x-show="open" x-cloak
        ...
        class="notification-bell-dropdown absolute {{ $align === 'left' ? 'left-0' : 'right-0' }} top-full mt-2 w-[26rem] max-w-[calc(100vw-2rem)] bg-white rounded-xl border border-gray-100 shadow-2xl z-[60] overflow-hidden">
