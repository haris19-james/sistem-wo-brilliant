function adminVendorDirectory(config) {
  return {
    ...
    async fetchCards(reset = false) {
      ...
      if (reset) {
        this.page = 1;
        _prevVendors = Array.isArray(this.vendors) ? this.vendors.slice() : [];
        if (this._observer) this._observer.disconnect();
      ...
      try {
        ...
        if (!Array.isArray(rows)) throw new Error('Invalid response shape: data array expected');

        const normalized = rows.map((r, i) => {
          if (r == null || typeof r !== 'object') return r;
          if (r.id == null) r.id = `_tmp_${fetchPage}_${i}_${Date.now()}`;
          return r;
        });

        this.lastPage = json.meta?.last_page ?? 1;
        this.total = json.meta?.total ?? (this.vendors.length + normalized.length);

        if (reset) {
          this.vendors = normalized;
          this.page = normalized.length ? 2 : 1;
        } else {
          this.vendors = [...this.vendors, ...normalized];
          this.page = fetchPage + 1;
        }
      } catch (e) {
        console.error(e);
        this.errorMessage = 'Gagal memuat data. Silakan coba lagi.';
        if (reset && _prevVendors) this.vendors = _prevVendors;
      } finally {
        this.loading = false;
    ...
    retryLoadMore() {
      this.errorMessage = null;
      this.fetchCards(this.vendors.length === 0);
    },
